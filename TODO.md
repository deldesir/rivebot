...
- [ ] **`sysadmin persona default <slug>`** — Runtime override of DEFAULT_PERSONA without env restart (command surface is now `sysadmin …`, not `#persona`)
...
- [ ] **`sysadmin persona create` options** — Support setting `language` and `allowed_tools` in the create command
